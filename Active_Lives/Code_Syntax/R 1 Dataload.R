...
age <- read.csv("C:/Annes offline R/Sport England/02 Data_Images/census 2021 age.csv")
car <- read.csv("C:/Annes offline R/Sport England/02 Data_Images/census 2021 car or van.csv")
ethnic <- read.csv("C:/Annes offline R/Sport England/02 Data_Images/census 2021 ethnic group.csv")

hours <- read.csv("C:/Annes offline R/Sport England/02 Data_Images/census 2021 hours worked.csv")
lang <- read.csv("C:/Annes offline R/Sport England/02 Data_Images/census 2021 main language.csv")
lone_parent <- read.csv("C:/Annes offline R/Sport England/02 Data_Images/census 2021 household type.csv")
limited_act <- read.csv("C:/Annes offline R/Sport England/02 Data_Images/census 2021 day-to-day activities are limited a lot.csv")
hh_comp <- read.csv("C:/Annes offline R/Sport England/02 Data_Images/census 2021 household composition.csv")
carer <- read.csv("C:/Annes offline R/Sport England/02 Data_Images/census 2021 unpaid carer.csv")
sr_health <- read.csv("C:/Annes offline R/Sport England/02 Data_Images/census 2021 health sr.csv")

dep_edu <- read.csv("C:/Annes offline R/Sport England/02 Data_Images/census 2021 deprived in education dimension.csv")
dep_hh <- read.csv("C:/Annes offline R/Sport England/02 Data_Images/census 2021 deprived in hh dimension.csv")
dep_econ <- read.csv("C:/Annes offline R/Sport England/02 Data_Images/census 2021 economic activity inc retired.csv")
dep_health <- read.csv("C:/Annes offline R/Sport England/02 Data_Images/census 2021 dep heath dim.csv")

imd_all <- read.csv("C:/Annes offline R/Sport England/02 Data_Images/File_7_-_All_IoD2019_Scores__Ranks__Deciles_and_Population_Denominators_3.csv")
lsoa_lookup <- read.csv("C:/Annes offline R/Sport England/02 Data_Images/LSOA_(2011)_to_LSOA_(2021)_to_Local_Authority_District_(2022)_Exact_Fit_Lookup_for_EW_(V3).csv")

rurality <- read.csv("C:/Annes offline R/Sport England/02 Data_Images/Rural_Urban_Classification_(2021)_of_LSOAs_in_EW.csv")
green <- read_excel("C:/Annes offline R/Sport England/02 Data_Images/ons green space access reduced version.xlsx", sheet = "Eng_Wal") #Scotland removed
activity <- read_excel("C:/Annes offline R/Sport England/02 Data_Images/summary_activity_levels.xlsx")
...
# Age columns to sum for proportion once pivoted
age_cols <- c(
...
#Pivot wide then calculate % children
age_wide <- age %>%
  # Pivot wider
  pivot_wider(
...
#colnames(dep_econ)
#unique(dep_econ$Economic.activity.status..10.categories.)
#colnames(dep_econ_wide)
# Age columns to sum for proportion once pivoted
dep_econ_cols <- c(
  "-8_Does not apply",
...
#colnames(ethnic)
#unique(ethnic$Economic.activity.status..10.categories.)
#colnames(ethnic_wide)
# Age columns to sum for proportion once pivoted
ethnic_cols <- c(
  "-8_Does not apply"  ,                                                      
...
#colnames(lang)
#unique(lang_wide$Main.language..11.categories.)
#colnames(lang_wide)
# Age columns to sum for proportion once pivoted
lang_cols <- c(
  "-8_Does not apply"  ,                                                      
...
#people whose daily activities are restricted because of a long-term physical or mental health condition, illness, or disability
#colnames(limited_act)
#colnames(limited_act_wide)
# Age columns to sum for proportion once pivoted
limited_act_cols <- c(
  "-8_Does not apply"  ,                                                      
...
#educational disadvantage is distributed geographically - lower adult skills and poorer school outcomes.
#Analysing links between education deprivation and health outcomes, LTC prevalence, or economic inactivity.

#colnames(dep_edu)
#colnames(dep_edu_wide)
# Age columns to sum for proportion once pivoted
dep_edu_cols <- c(
  "-8_Does not apply"  ,                                                      
...
#write_xlsx(starter_table, "starter_table.xlsx")
