...
#---------------------------------------------------------------
# Histograms and density plots for numeric data
#---------------------------------------------------------------
...
for(col in names(numeric_cols)) {
  p <- ggplot(combined_lsoa_indicators, aes_string(x = col)) +
    ...
    scale_x_continuous(breaks = pretty(combined_lsoa_indicators[[col]], n = 15)) +  
    scale_y_continuous(breaks = scales::pretty_breaks(n = 10), labels = scales::number_format(accuracy = 1))+
                                    #for smooth estimate of distribution
...
#-------------------------------------------------------------------------------
# Correlation with numeric variable against the rural urban flag
...
#`active_aged 75+`     - predictor/target variable
...
#slightly higher correlation with log_prop_dep_hh:
...
#Urban_rural_flag      - used to understand level of correlation with other variables but RUC21NM encompasses 
                          # greater detail about distances to urban areas and remoteness of areas.  This informs 
